...
class RisultatiTS1 extends StatefulWidget {
  const RisultatiTS1({super.key});

  @override
  State<RisultatiTS1> createState() => _RisultatiTS1State();
}

class _RisultatiTS1State extends State<RisultatiTS1> {
      ...
      for (var i = 1; i < lines.length; i++) {
        if (lines[i].trim().isNotEmpty) {
          try {
            ...
            if (parts.length >= 9) { // Verifica che ci siano tutti i campi necessari
  ...
  void _exportToCsv() {
    try {
      ...
      String csvContent = 'Numero Rigo;Data;Metodo;Ciclo;Risultato;U.M.;ODS;ODE;Note\n';
      ...
      final anchor = html.AnchorElement(href: url)
        ..setAttribute("download", "risultati_ts1.csv")
        ..click();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Risultati TS 1.x.x'),
        centerTitle: true,
      ),
      body: Container(
